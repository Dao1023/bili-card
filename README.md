...
- **Live Preview**：连续的卡片行自动合并成画廊并排显示；点卡片打开链接，点画廊右上角 `</>` 按钮（或空白处）进入源码编辑态。编辑态顶部工具栏：通过 URL 添加卡片、按日期排序（往复切换 新→旧 / 旧→新，无日期的排最后）、转回链接（卡片还原成普通 markdown 链接，写回后画廊消失），改完"完成"统一写回。
- **阅读模式**：同样渲染成卡片。
