...
**最快的方式：命令面板或编辑器右键菜单 → `插入 B 站卡片`**，贴入视频链接/BV号或 UP 主空间链接/mid，插件自动拉取标题、封面、播放量等信息并插入卡片。支持批量：链接每行一个，逐条拉取，失败的跳过。UP 主卡可另填代表作视频链接（仅单条时生效）。在画廊的源码编辑态（`</>`）里也有"通过 URL 添加"按钮，新卡追加进当前画廊。
...
- **Live Preview**：连续的卡片行自动合并成画廊并排显示；点卡片打开链接，点画廊右上角 `</>` 按钮（或空白处）进入源码编辑态。编辑态是内嵌 CodeMirror,带 HTML 语法高亮（配色跟随主题代码高亮变量）和行号。编辑态顶部工具栏：通过 URL 添加卡片、按日期排序（往复切换 新→旧 / 旧→新，无日期的排最后）、转回链接（卡片还原成普通 markdown 链接，写回后画廊消失），改完"完成"统一写回。
- **阅读模式**：同样渲染成卡片。
